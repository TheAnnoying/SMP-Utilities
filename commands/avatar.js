...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        let boolean = message.options.getBoolean('guild')
        if(boolean === true){
            let guild = message.guild;
            const guildAvatarEmbed = new Discord.MessageEmbed()
            .setTitle(`${message.guild.name}'s Icon:`)
            .setDescription(`[16x](${guild.iconURL({ size: 16, dynamic: true, format: 'png' })}), [32x](${guild.iconURL({ size: 32, dynamic: true, format: 'png' })}), [64x](${guild.iconURL({ size: 64, dynamic: true, format: 'png' })}), [128x](${guild.iconURL({ size: 128, dynamic: true, format: 'png' })}), [256x](${guild.iconURL({ size: 256, dynamic: true, format: 'png' })}), [512x](${guild.iconURL({ size: 512, dynamic: true, format: 'png' })}), [1024x](${guild.iconURL({ size: 1024, dynamic: true, format: 'png' })})`)
            .setImage(guild.iconURL({ size: 1024, format: 'png', dynamic: true }))
            .setColor(bot.accentColor)
            .setFooter({ text: "This is a guild icon!" });
            return message.reply({ embeds:[guildAvatarEmbed] });
        }
