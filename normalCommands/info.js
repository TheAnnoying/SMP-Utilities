...
module.exports.run = async(client, message, args) => {
        if(args[0] === 'user'){
            args = [].concat(args.slice(0, 1), args.slice(1, args.length).join(" "))
            let target = message.mentions.members.first() || (await message.guild.members.fetch()).find(member => member.displayName.toLowerCase() === args[1].toLowerCase()) || (await message.guild.members.fetch()).find(member => member.user.username.toLowerCase() === args[1].toLowerCase()) || await message.guild.members.fetch(args[1]);
        ...
        if(args[0] === 'role'){
            let roleVar = await message.guild.roles.cache.find(r => r.name === args[1]) || await message.guild.roles.cache.find(r => r.id === args[1]);
            if(!roleVar) return message.reply({ embeds: [fn.sendError('You need to enter a valid role name / ID!')] });
        ...
        if(args[0] !== 'channel' && args[0] !== 'role' && args[0] !== 'server' && args[0] !== 'user') return message.reply({ embeds: [fn.sendError("You need to type at least one type of info! (\`user\`, \`server\`, \`role\` and \`channel\`)")] })
}
